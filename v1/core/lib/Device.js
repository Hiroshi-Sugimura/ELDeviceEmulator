...
// 設定情報が更新されたときの処理
Device.prototype.updateConf = function (conf) {
	// 変更があったパラメータ名のリスト
...
Device.prototype._initNodeProfileInstance = function (devobj) {
...
Device.prototype._findFilesToDeleteForReset = function (dpath) {
	let promise = new Promise((resolve, reject) => {
...
Device.prototype._deleteFiles = function (fpath_list) {
	let promise = new Promise((resolve, reject) => {
...
// IPv6 モードならマルチキャスト送信のネットワークインタフェースをセット
Device.prototype.setMulticastInterface6 = function () {
	if (this._conf['ip_address_version'] !== 6) {
		return;
	}
...
Device.prototype._sendInstanceListNotification = function (node_profile_eoj, eoj_list, callback) {
	if (!callback) {
		callback = () => { };
	}
...
Device.prototype._sendPropertyNotification = function (eoj_list) {
	// eoj_list = ["000D01"]
...
// EL パケットを受信したときの処理
Device.prototype._receivePacket = function (buf, device_info) {
	// 送信元アドレス
...
Device.prototype._addMembership = function () {
	/*
...
Device.prototype._dropMembership = function () {
	/*
	if(this._conf['ip_address_version'] !== 4) {
		return;
	}
...
Device.prototype._sendSnaForOpcOverflow = function (address, parsed) {
...
Device.prototype._convHexToBuffer = function (hex) {
...
//   リモートデバイス発見処理
Device.prototype._receivePacketForController = function (parsed, device_info) {
	if (!this._is_controller) {
		return;
	}
...
Device.prototype._isSameEojList = function (r1, r2) {
	let eojs2 = {};
...
Device.prototype._getRemoteDevicePropertyMaps = function (address, eoj_list) {
...
Device.prototype._getRemoteDeviceEojPropertyMaps = function (address, eoj, callback) {
...
Device.prototype._getRemoteDeviceReleases = function (address, eoj_list) {
	let eoj_code_list = JSON.parse(JSON.stringify(eoj_list));
	let promise = new Promise((resolve, reject) => {
		let rels = {};
		let getRelease = () => {
...
Device.prototype._getRemoteDeviceManufacturerCodes = function (address, eoj_list) {
	let eoj_code_list = JSON.parse(JSON.stringify(eoj_list));
	let promise = new Promise((resolve, reject) => {
		let manus = {};
...
Device.prototype._request = function (address, packet, release) {
	let promise = new Promise((resolve, reject) => {
...
Device.prototype._waitPromise = function (msec) {
	let promise = new Promise((resolve, reject) => {
		setTimeout(() => {
			resolve();
		}, msec);
