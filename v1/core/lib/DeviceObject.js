...
DeviceObject.prototype.updateConf = function (conf) {
...
* - リリースバージョン (例: "J")
* ---------------------------------------------------------------- */
DeviceObject.prototype.getStandardVersion = function () {
...
* }
* ---------------------------------------------------------------- */
DeviceObject.prototype.getAccessRule = function (epc) {
...
* ---------------------------------------------------------------- */
DeviceObject.prototype.getEpcValues = function (props, is_admin) {
...
* reject() は本メソッドに渡されたパラメータに不備があった場合のみ呼び出される。
* ---------------------------------------------------------------- */
DeviceObject.prototype.setEpcValues = function (props, is_admin, set_delay_msec) {
...
* ---------------------------------------------------------------- */
DeviceObject.prototype.receive = function (address, parsed) {
...
// SetI 書き込み要求：応答不要
DeviceObject.prototype._receiveReqSetI = function (address, packet) {
	let epc_list = [];
...
// SetC 書き込み要求：応答要
DeviceObject.prototype._receiveReqSetC = function (address, packet) {
	let epc_list = [];
	packet['properties'].forEach((o) => {
		epc_list.push(o['epc']);
	});
...
DeviceObject.prototype._sendStatusChangeInf = function (packet, changed) {
...
// Get 読み出し要求
DeviceObject.prototype._receiveReqGet = function (address, packet) {
...
DeviceObject.prototype._getSetDelayMsec = function (epc_list) {
...
DeviceObject.prototype._getResponseWaitMsec = function (req_packet, epc_list) {
	if (!epc_list || !Array.isArray(epc_list)) {
		epc_list = [];
	}
...
DeviceObject.prototype._isMulticastRequest = function (req_packet) {
	let p = req_packet;
...
// INF_REQ 通知要求
DeviceObject.prototype._receiveReqInfReq = function (address, packet) {
	this._states.getEpcValues(packet['properties']).then((res) => {
		/*
		* res = {
		*   result  : 読み出しに失敗した EDT の数 (つまりすべて成功すれば 0),
...
// SetGet 書き込み・読み出し要求
DeviceObject.prototype._receiveReqSetGet = function (address, packet) {
...
// 0x74 INFC 通知：応答要
DeviceObject.prototype._receiveReqInfC = function (address, packet) {
	let props = {};
	let epc_list = [];
	packet['properties'].forEach((p) => {
		props[p['epc']] = null;
...
DeviceObject.prototype._createSnaPacket = function (packet, props) {
	/* ----------------------------------------------------------------
	* packet: リクエストのパケット
...
DeviceObject.prototype._createResPacket = function (packet, props) {
	/* ----------------------------------------------------------------
	* packet:
	*   - tid        | integer | optional | 指定がなけれは自動採番
	*   - seoj       | string  | required | 16進数文字列 (例: "013001")
