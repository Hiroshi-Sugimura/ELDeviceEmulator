...
* 初期化する
* ---------------------------------------------------------------- */
DeviceState.prototype.init = function () {
	// EPC の初期値
	let vals = {};
...
// 初期値を確定する
DeviceState.prototype._getInitialHexValue = function (d) {
	if (!d) {
		return null;
	}
...
// 個別の初期値をセットする
DeviceState.prototype._setSpecialInitialHexValue = function (vals) {
	// 規格Version 情報 (EPC: 0x82) をセットする
	if (vals['82']) {
		if (/^0EF0/.test(this._eoj)) {
			vals['82'] = '010C0100'; // V1.12
		} else {
			let vhex = this._standard_version.charCodeAt(0).toString(16);
...
DeviceState.prototype._createElectricEnergyLogData = function () {
...
DeviceState.prototype._createElectricEnergyHistoricalData = function () {
...
// プロパティマップをセットする
DeviceState.prototype._setPropertyMaps = function () {
...
DeviceState.prototype._setPropertyMap = function (pmap_epc, epc_list) {
...
DeviceState.prototype._readStateFileSync = function () {
	let o = null;
...
DeviceState.prototype._writeStateFileSync = function () {
	try {
...
DeviceState.prototype._writeStateFileDelay = function (delay_msec, cvals) {
	let retry = 0;
	let saveFile = () => {
		if (this._flock === true) {
			retry++;
...
// 状態ファイルを書きだす (非同期)
DeviceState.prototype._writeStateFile = function () {
...
// デバッグ用
DeviceState.prototype.getAllEpcValues = function () {
...
* }
* ---------------------------------------------------------------- */
DeviceState.prototype.getAccessRule = function (epc) {
...
* ---------------------------------------------------------------- */
DeviceState.prototype.getEpcValues = function (props, is_admin) {
	let promise = new Promise((resolve, reject) => {
...
DeviceState.prototype._convHexToBuffer = function (hex) {
...
DeviceState.prototype._getSpecialEdt = function (epc) {
	// スーパークラスの個別処理
	if (epc === '97') { // 現在時刻設定
...
// 現在時刻設定 (EPC: 0x97) を取得
DeviceState.prototype._getEdt97 = function () {
...
DeviceState.prototype._getEdt98 = function () {
	let now = Date.now() + this._time_diff;
	let dt = new Date(now);
	let Y = dt.getFullYear();
	let M = dt.getMonth() + 1;
...
// 積算運転時間 (EPC: 0x9A) を取得
DeviceState.prototype._getEdt9A = function () {
...
// EPC のアクセスルールをチェック
DeviceState.prototype._checkEpcPermission = function (req_type, epc) {
	// 存在する EPC かどうかをチェック
...
* reject() は本メソッドに渡されたパラメータに不備があった場合のみ呼び出される。
* ---------------------------------------------------------------- */
DeviceState.prototype.setEpcValues = function (props, is_admin, set_delay_msec) {
...
DeviceState.prototype._setElectricEnergyHistoricalData = function (uvals, cvals) {
	let save_vals = {};
...
DeviceState.prototype._setEpcForReset = function (uvals, cvals) {
...
DeviceState.prototype._setEdt97 = function (hex) {
	if (!hex || hex.length !== 4) {
		return null;
	}
	let now = Date.now() + this._time_diff;
...
DeviceState.prototype._setEdt98 = function (hex) {
	if (!hex || hex.length !== 8) {
		return null;
	}
	let now = Date.now() + this._time_diff;
...
DeviceState.prototype._isValidEdt = function (val, pdata, is_admin) {
